...
6. **Delegate implementation to a sonnet sub-agent.** Use the Agent tool with `subagent_type: "claude"` and `model: "sonnet"`. Brief it with: files to touch, the interfaces/tests you wrote, the guardrails it must pass.
...
When spawning the sonnet implementer, include:
