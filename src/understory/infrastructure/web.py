...
def _render_index(sessions: list[Session]) -> str:
    if not sessions:
        rows = "<tr><td colspan=5>No sessions recorded yet.</td></tr>"
    else:
        rows = "".join(
            "<tr>"
            f'<td><a href="/{html.escape(s.id)}">{html.escape(s.id)}</a></td>'
    ...
    table = (
        "<table>"
        "<tr><th>Session ID</th><th>Model</th><th>Status</th><th>Steps</th><th>Task</th></tr>"
        f"{rows}"
        "</table>"
    )
...
def _render_detail(session: Session) -> str:
    ...
    body = f"<h1>Session {html.escape(session.id)}</h1>{header}{steps_html}"
    return _page(f"Session {session.id}", body)
...
def build_web_app(store: TraceStore) -> Starlette:
    ...
    async def index(request: Request) -> Response:
        return HTMLResponse(_render_index(list(store.list())))
